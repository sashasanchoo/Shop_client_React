...
export default function AdminPanelDeleteConcreteCommentPage(){
    ...
    const submit = async (e) => {
        e.preventDefault()
        await fetch(`https://localhost:7104/api/Comments/${state.comment.id}`, {
            method: "DELETE",
            headers: {
            ...
            if(response.status === 204){
                ...
                navigate('/Account/Manage/Admin/Products/Details', {state:{id: state.product.id}})
            }
            ...
            (
                <form onSubmit={submit}>
                    <div className={'form-group'}>
                        <label className={'form-label'}>Are you sure you want to delete "{state.comment.content}" comment ?</label>
                    </div>
                    <span className={errorHolderClassName}>{localRequestResultHolder}</span>
                    <div className={'form-floating'}>
                        <button className={'btn btn-danger'} type={'submit'}>Delete</button>
                        <NavLink className={'btn btn-secondary'} to='/Account/Manage/Admin/Products/Details' state={{id: state.product.id}}>
                            Back to list
                        </NavLink>
                    </div>
                </form>
